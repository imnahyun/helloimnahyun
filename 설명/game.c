...
void random_numbers(int random[], int hint[]) // 랜덤 숫자와 힌트를 생성하는 함수
{
    ...
    for (int index = 0; index < 2; index++) // 힌트를 생성하기 위한 for 루프
    {
        ...
        if (index==1 && hint[0] == hint[1]) //왜 index==1? 해결~~
            index--;

    }
}
void get_user(int user[], char usernumber[]) // 유저로부터 입력받는 함수
{
    while (1) // 유저가 제대로 입력할 때까지 무한 루프를 돌림
    {
        ...
        for (int i = 0; i < 3; i++)
        {
            ...
            //'0'을 뺀 값이 실제 숫자 값입니다.
            user[i] = usernumber[i] - '0';//문자열로 숫자를 받고, 0~9까지 랜덤으로 받은뒫에, 맨마지막에 아스키코드 0을 빼서 정수로 만들어준다.
        }
...
void play_game(int random[], int hint[], int user[], int* chance, int* try_count, int* game_over)
{
    ...
    if (strike == 3)
    {
        ...
        *game_over = 1;//*는 포인터로 불러오는걸 의미한다.그러니까 1은 참이니까 이 구문을빠져나갑니다. 원래는 0고 0면 게임실패니까 빠져나가지못하고 페일!
        return;
    }
